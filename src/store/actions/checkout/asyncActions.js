...
export const submitShippingAddress = (payload = {}) =>
  async function thunk(dispatch, getState) {
    ...
    const { formValues, countries, setGuestEmail, setShippingAddressOnCart } = payload;
    ...
    try {
      ...
      const { firstname, lastname, street, city, region_code, postcode, telephone, country_id } = address;
      ...
      const shippingMethods = data.setShippingAddressesOnCart.cart.shipping_addresses[0].available_shipping_methods;
...
export const createAccount =
  ...
  async (dispatch, getState) => {
    ...
    const { email, firstname: firstName, lastname: lastName } = checkout.receipt.order.shipping_address;
